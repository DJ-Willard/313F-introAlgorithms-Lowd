...
class rb_tree(object):
    ...
    def left_rotate(self, current_node):
        ...
        # //Rotation is the step in the red black tree where the parent becomes the child
        # //and the child the parent. in Left rotate the Right Child becomes the Parent
        # //and the Parent becomes the Left Child.
        # // t.nil = self.sential
    ...
    def right_rotate(self, current_node):
        ...
        # T2 becomes left child of y and T3 becomes right child of y
            # refer page 328 of CLRS book for rotations
        # //Rotation is the step in the red black tree where the parent becomes the child
        # //and the child the parent. In Right rotate the Left Child becomes the Parent
        # //and the Parent becomes the Right Child.
        if(current_node == None):
            raise KeyError
        y = current_node
        if (y.left == self.sentinel):
    ...
    def __rb_insert_fixup(self, z):
        ...
        # // after inserting a node as a red node this may cause a break in the RB-tree properties
        # // there are 3 cases that require a fix and are listed below
        # 0) Emtpy or only root insert is correct
        # 1) z's uncle is red (requires recolour or z parent and uncle and possible the root of the sub tree)
        # 2) z's uncle is black and the child in to the inside of the tree (requires a rotation to create a line)
        # this leads us to case
        # 3) z's uncle is black and a line is formed (rotation and then a recolour is required)
    ...
    def __rb_delete_fixup(self, x):
        ...
        # from the tree. Please read the code for delete() method to get a better understanding.
            # refer page 342 of CLRS book and lecture slides for rb_delete_fixup
        # // after deleting a node in a red black tree this may cause a break in the RB-Tree Properties
        # // there are 4 cases that require clean up after a delete
        # 1) w is red with an uneven height of black nodes on left and right of w's parent
        # (this will require  a recolour of w and a rotate) this can lead to case 2
        # 2) w' or w is black uneven height of black nodes
        # (requires a recolor of w' or w to red)
        # 3) w is black and it subtree is uneven in black nodes
        # (requires a recolour in the subtree and w and then a rotate to a line)
        # this can lead to a case 4
        # 4) w is black and its child is red and it sibling is black
        # (this requires a recolour and rotate)
